...
move /Y "%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster\*.config" c:\tmpUlises\cluster
pause

IF /i NOT EXIST "%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster" GOTO seguir
...
"%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster\ClusterSrv" -uninstall
ECHO.
ECHO El servicio ClusterSrv ha sido eliminado
)

:seguir
@xcopy /EIY ".\Cluster\Servidor" "%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster"
"%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster\ClusterSrv" -install
...
echo **** Restore config
move /Y "c:\tmpUlises\cluster\*.config" "%PROGRAMFILES(X86)%\DF Nucleo\UlisesV5000Cluster"
...
:end
@echo **** Fin de la instalacion de los servicios de cluster
